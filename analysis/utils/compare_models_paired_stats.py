...
Hyperparameters are documented under mref-ad/configs/best_hyperparameters/; this script
...
Paper setup (proposed model vs baselines):
  Default is --primary moe (your MoE) vs --baselines lr,rf,mlp,xgb,ftt,flex_moe — i.e. classical
  baselines plus Flex-MoE, all on the same 10 split seeds. One-sided tests support the claim
  "MoE > baseline"; two-sided tests only ask whether paired scores differ.

Example:
  python3 scripts/compare_models_paired_stats.py
...
def load_moe(repo: Path) -> dict[int, dict[str, float]]:
...
def get_loaders(repo: Path, flex_moe_json: str) -> dict[str, Callable[[Path], dict[int, dict[str, float]]]]:
    return {
        ...
        "lr": lambda r: load_summary_per_seed(r, "results/lr_seed7_bestparams_all10/summary.json"),
        "rf": lambda r: load_summary_per_seed(r, "results/rf_seed7_bestparams_all10/summary.json"),
        "mlp": lambda r: load_summary_per_seed(r, "results/mlp_seed7_bestparams_all10/summary.json"),
        "xgb": lambda r: load_summary_per_seed(r, "results/xgb_seed7_no_retrain_best_all10/summary.json"),
        "ftt": load_ftt,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="moe",
        choices=["moe", "flex_moe", "lr", "rf", "mlp", "xgb", "ftt"],
        help="Model treated as the method under test (default: moe).",
    )
    parser.add_argument(
        "--baselines",
        type=str,
    ...
    report: dict[str, Any] = {
        ...
        "hyperparameter_note": (
            "Tuned hyperparameters are recorded in mref-ad/configs/best_hyperparameters/ "
